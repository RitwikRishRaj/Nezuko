...
class ArenaController {
  ...
  async createSession(req, res, next) {
    try {
      ...
      // Create arena session - match actual database schema
      const sessionData = {
        room_id: roomId,
        questions: questions,
        ...
        custom_problem_links: customProblemLinks || null,
        use_custom_links: useCustomLinks || false
        // Note: 'config' and 'created_by' columns don't exist in arena_sessions table
      };
      ...
      // Validate that we have either questions or custom links
      if ((!sessionData.questions || sessionData.questions.length === 0) && 
          (!sessionData.custom_problem_links || sessionData.custom_problem_links.length === 0)) {
        console.warn('⚠️ Arena session created with no questions or custom links!');
      }
