...
const roomConfigSchema = Joi.object({
  ...
  roomMode: Joi.string().valid('1v1', 'team-vs-team').default('team-vs-team')
});
...
class RoomController {
  ...
  async getConfig(req, res, next) {
    try {
      ...
      const [config, invites] = await Promise.all([
        db.getRoomConfig(roomId),
        db.getRoomInvites(roomId)
      ]);
      ...
      // Get room_mode from invites (since it's stored there, not in config)
      const roomMode = invites && invites.length > 0 ? invites[0].room_mode : 'team-vs-team';

      // Add room_mode to config for frontend compatibility
      const configWithRoomMode = { ...config, room_mode: roomMode };

      res.json({ config: configWithRoomMode });
    } catch (err) {
      next(err);
    }
  }
  ...
  async getRoomState(req, res, next) {
    try {
      ...
      // Get both invites and room configuration
      ...
      // Add room_mode to config for frontend compatibility
      const configWithRoomMode = config ? { ...config, room_mode: roomMode } : null;

      res.json({ 
        invites,
        config: configWithRoomMode // Include room config with room_mode so participants can see questions and mode
      });
    } catch (err) {
      next(err);
    }
  ...
  async startGame(req, res, next) {
    try {
      ...
      // Update room_config with game start info and questions
      await db.updateRoomGameStatus(roomId, {
        game_status: 'started',
        game_started_by: req.userId,
        game_started_at: gameStartTime,
        problems: questions || [], // Store questions in room config as backup
        custom_problem_links: finalCustomProblemLinks || null,
        use_custom_links: finalUseCustomLinks || false
      });
      ...
      try {
        ...
        // First check if arena service is reachable
        try {
          const healthCheck = await fetch(`${arenaServiceUrl}/health`, { timeout: 5000 });
          console.log('🏥 Arena service health check:', healthCheck.status);
        } catch (healthError) {
          console.error('🏥 Arena service health check failed:', healthError.message);
        }
        ...
        console.error('❌ Failed to create arena session via arena-service:', arenaError.message);
        console.error('❌ Arena error details:', arenaError);
        
        // Fallback: Create arena session directly from room-service
        console.log('🔄 Attempting fallback: Creating arena session directly...');
        try {
          const sessionData = {
            room_id: roomId,
            questions: questions || [],
            ...
            end_time: new Date(Date.now() + (roomConfig.minutes * 60 * 1000)).toISOString()
            // Note: custom_problem_links and use_custom_links are optional
          };
          
          const session = await db.createArenaSessionFallback(sessionData);
          
          // Create participant records
          for (const participant of participants) {
            await db.createParticipantFallback({
              arena_session_id: session.id,
              participant_clerk_id: participant.clerk_id,
              team_type: participant.team_type
              // Note: participant_handle column may not exist in participant_progress table
            });
          }
          
          console.log('✅ Fallback arena session created successfully:', session.id);
          ...
          res.json({
            ...
            participants,
            arenaSession: null,
            warning: `Arena session creation failed: ${fallbackError.message}`
          });
        }
      }
    } catch (err) {
