...
export default function VerifyPage() {
  ...
  // Check if user is admin or already verified and redirect accordingly
  useEffect(() => {
    const checkUserStatus = async () => {
      ...
      // If user is admin, create admin user and redirect immediately without verification
      if (isAdminEmail(userEmail)) {
        try {
          // Create admin user in database without Codeforces verification
          const createAdminResponse = await fetch(API_CONFIG.BASE_URL + API_CONFIG.ENDPOINTS.USER.CREATE, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              clerkId: userId,
              codeforcesHandle: 'admin', // Default handle for admin
              rating: 0 // Default rating for admin
            })
          });

          if (!createAdminResponse.ok) {
            // If user already exists, that's fine, just redirect
            console.log('Admin user might already exist, proceeding with redirect');
          }
        } catch (error) {
          console.log('Error creating admin user, but proceeding with redirect:', error);
        }
      ...
      // For non-admin users, check if they already exist in database
      try {
        const response = await apiClient.get(API_CONFIG.ENDPOINTS.USER.DETAILS, { clerkId: userId });
        
        if (response.ok) {
          // User already exists and verified, redirect to regular dashboard
          window.location.href = getRegularUserRedirectUrl();
          return; // Don't set loading to false since we're redirecting
        }
        // If user doesn't exist (404), stay on verify page for Codeforces verification
      } catch (error) {
        console.log('User not found in database, staying on verify page for verification');
        // User doesn't exist, stay on verify page for Codeforces verification
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const createUserResponse = await fetch(API_CONFIG.BASE_URL + API_CONFIG.ENDPOINTS.USER.CREATE, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          clerkId: userId, // This will be used by auth middleware
  ...
  // Show loading while checking if user already exists
