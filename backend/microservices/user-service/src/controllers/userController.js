...
const createUserSchema = Joi.object({
  clerkId: Joi.string().required(), // Required for auth middleware
...
class UserController {
  async createUser(req, res, next) {
    try {
      ...
      const { clerkId, codeforcesHandle, rating } = value;
      
      // Ensure the clerkId matches the authenticated user
      if (clerkId !== req.userId) {
        return res.status(403).json({ error: 'ClerkId mismatch with authenticated user' });
      }

      const userData = {
        clerk_id: req.userId,
        codeforces_handle: codeforcesHandle,
        codeforces_rating: rating || 0,
        current_rating: rating || 0,
        peak_rating: rating || 0,
      ...
      if (existingUser) {
        ...
        user = await db.updateUser(req.userId, {
          ...
          codeforces_rating: rating || 0,
          current_rating: rating || 0,
          peak_rating: Math.max(rating || 0, existingUser.peak_rating || 0),
          is_verified: true,
          updated_at: new Date().toISOString()
        });
      } else {
